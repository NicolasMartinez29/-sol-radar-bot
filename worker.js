...
//
// Commands:
// /start   -> activate radar
// /stop    -> deactivate radar
// /status  -> bot status
...
// - Telegram token is stored as a Cloudflare secret.
// ============================================================
...
async function runRadar(env) {
  ...
  console.log("SOL RADAR SCAN STARTED");
  ...
  try {
    ...
    // Manage existing paper trades first
    await updateOpenTrades(state, market);

    // Avoid excessive simultaneous positions
    ...
    // Look for new setup
    const signal = generateSignal(market, state);
    ...
    await sendTelegramSignal(env, signal, trade);
...
// ============================================================
//
// Uses CoinGecko public API for initial paper-trading version.
// Later we can replace this with a dedicated Solana market-data
...
async function getMarketData() {
  ...
  if (prices.length < CONFIG.LOOKBACK) {
    return null;
  }
  ...
  for (let i = 0; i < prices.length; i++) {
    candles.push({
      ...
      volume: volumes[i] ? Number(volumes[i][1]) : 0
    });
  }

  return {
    candles,
    price: candles[candles.length - 1].price
  };
}
...
function generateSignal(market, state) {
  ...
  const fastMA = average(closes.slice(-9));
  const slowMA = average(closes.slice(-21));

  const previousFastMA = average(closes.slice(-10, -1));
  const previousSlowMA = average(closes.slice(-22, -1));
  ...
  const oldVolume = average(volumes.slice(-20));
  ...
  const bullishTrend = fastMA > slowMA;
  const bearishTrend = fastMA < slowMA;
  ...
  const previousHigh =
    Math.max(...closes.slice(-12, -1));

  const previousLow =
    Math.min(...closes.slice(-12, -1));
  ...
  if (bullishTrend) longScore += 25;
  if (bearishTrend) shortScore += 25;

  if (bullishBreakout) longScore += 25;
  if (bearishBreakout) shortScore += 25;

  if (momentum > 0.35) longScore += 20;
  if (momentum < -0.35) shortScore += 20;

  if (volumeRatio > 1.25) {
    longScore += bullishTrend ? 15 : 0;
    shortScore += bearishTrend ? 15 : 0;
  }

  if (rangePercent > 1) {
    longScore += bullishTrend ? 10 : 0;
    shortScore += bearishTrend ? 10 : 0;
  }

  const isLong = longScore >= CONFIG.MIN_SCORE;
  const isShort = shortScore >= CONFIG.MIN_SCORE;
  ...
  const volatility =
    Math.max(rangePercent / 100, 0.005);

  const stopDistance =
    price * volatility * 0.65;
...
function createPaperTrade(signal) {
  ...
  const riskPerUnit =
    Math.abs(signal.entry - signal.stop);
...
async function updateOpenTrades(state, market) {
...
async function handleTelegram(request, env) {
  ...
  if (command === "/stop") {
    ...
    await telegramSend(
      env,
      chatId,
      "🛑 SOL RADAR DETENIDO 🔴\n\nNo se generarán nuevas señales hasta usar /start."
    );
...
async function sendTelegramSignal(env, signal, trade) {
...
function formatStatus(state) {
  ...
  return [
    ...
    `Last scan: ${
      state.lastScan
        ? new Date(state.lastScan).toLocaleString()
        : "Never"
    }`
  ].join("\n");
}
...
async function saveState(env, state) {
  ...
  state.signals =
    state.signals.slice(-500);

  state.closedTrades =
    state.closedTrades.slice(-500);
